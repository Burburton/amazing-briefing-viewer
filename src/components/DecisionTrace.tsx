...
function DecisionTrace({ decisions }: DecisionTraceProps) {
  if (decisions.length === 0) {
    return (
      <div className="briefing-card">
        <h3 className="text-h3">Decision History</h3>
        <p className="text-small text-briefing-muted">No decision requests recorded</p>
      </div>
    );
  }
  ...
  return (
    <div className="briefing-card">
      <h3 className="text-h3">Decision Email Trace</h3>
      ...
      {pending.length > 0 && (
        <div className="mb-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-small text-briefing-muted">Awaiting your reply</span>
          </div>
    ...
    <div className={`p-3 rounded border ${statusColors[decision.status]}`}>
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1">
          <p className="text-small font-medium text-briefing-secondary">
            {decision.question.slice(0, 100)}
            {decision.question.length > 100 && '...'}
          </p>
          <div className="flex items-center gap-2 mt-2">
            <span className={`px-2 py-0.5 rounded text-tiny ${
      ...
      {decision.status === 'pending' && decision.options && (
        <div className="mt-3 pt-3 border-t border-gray-100">
          <p className="text-tiny text-briefing-muted mb-2">Reply format</p>
          <p className="text-tiny text-briefing-secondary font-medium">DECISION {decision.recommendation || 'A'}</p>
        </div>
      )}

      {decision.status === 'resolved' && decision.resolution && (
        <div className="mt-3 pt-3 border-t border-gray-100">
          <p className="text-tiny text-green-600 font-medium">
